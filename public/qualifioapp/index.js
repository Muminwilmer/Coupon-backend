...
async function fetchAndExtractID(domain, pageUrl) {
  try {
    const response = await axios.get(`https://${domain}.se/${pageUrl}`)
...
async function automizeProcess(formData, domain, pageUrl) {
  try {
    // Step 1: Get them ids
    const id = await fetchAndExtractID(domain, pageUrl);
