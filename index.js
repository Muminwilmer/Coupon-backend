...
app.post('/qualifioapp/', async (req, res) => {
  ...
  const domain = req.body.domain.replace(/\S+\./, "");
  ...
  const extraPage = req.body.extraPage || ""

  try {
    const automizeProcess = require("./public/qualifioapp/index.js")
    const results = await automizeProcess(formData, domain, pageUrl, extraPage)
    const [id] = results
